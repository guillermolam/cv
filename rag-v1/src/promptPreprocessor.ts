...
async function getEffectiveContextFormatted(
  ...
  try {
    ...
    const hasAnyUserMessage = ctx
      .getMessagesArray()
      .some((message) => message.getRole() === 'user');
    if (!hasAnyUserMessage) {
      ...
      ctl.debug(text`
        ...
        user message.
      `);
      const measurementContext = ctx.withAppended(
        'user',
        placeholderUserMessageContent,
      );
...
async function measureContextWindow(
  ...
  const currentContextFormatted = await getEffectiveContextFormatted(
    ctx,
    model,
    ctl,
  );
