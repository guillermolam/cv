...
- [Application Variables Runtime Configuration](https://spinframework.dev/v3/dynamic-configuration#application-variables-runtime-configuration)
  - [Environment Variable Provider](https://spinframework.dev/v3/dynamic-configuration#environment-variable-provider)
  - [Vault Application Variable Provider](https://spinframework.dev/v3/dynamic-configuration#vault-application-variable-provider)
    - [Vault Application Variable Provider Example](https://spinframework.dev/v3/dynamic-configuration#vault-application-variable-provider-example)
  - [Azure Key Vault Application Variable Provider](https://spinframework.dev/v3/dynamic-configuration#azure-key-vault-application-variable-provider)
    - [Azure Key Vault Application Variable Provider Example](https://spinframework.dev/v3/dynamic-configuration#azure-key-vault-application-variable-provider-example)
- [Key Value Store Runtime Configuration](https://spinframework.dev/v3/dynamic-configuration#key-value-store-runtime-configuration)
  - [File Key Value Store Provider](https://spinframework.dev/v3/dynamic-configuration#file-key-value-store-provider)
  - [Redis Key Value Store Provider](https://spinframework.dev/v3/dynamic-configuration#redis-key-value-store-provider)
  - [Azure CosmosDB Key Value Store Provider](https://spinframework.dev/v3/dynamic-configuration#azure-cosmosdb-key-value-store-provider)
  - [AWS DynamoDB Key Value Store Provider](https://spinframework.dev/v3/dynamic-configuration#aws-dynamodb-key-value-store-provider)
  - [Multiple and Non-Default Key-Value Stores](https://spinframework.dev/v3/dynamic-configuration#multiple-and-non-default-key-value-stores)
- [SQLite Storage Runtime Configuration](https://spinframework.dev/v3/dynamic-configuration#sqlite-storage-runtime-configuration)
  - [LibSQL Storage Provider](https://spinframework.dev/v3/dynamic-configuration#libsql-storage-provider)
- [LLM Runtime Configuration](https://spinframework.dev/v3/dynamic-configuration#llm-runtime-configuration)
  - [Remote Compute Provider](https://spinframework.dev/v3/dynamic-configuration#remote-compute-provider)
    - [`default` API](https://spinframework.dev/v3/dynamic-configuration#default-api)
    - [`open_ai` API](https://spinframework.dev/v3/dynamic-configuration#open_ai-api)
- [Outbound HTTP Runtime Configuration](https://spinframework.dev/v3/dynamic-configuration#outbound-http-runtime-configuration)

You can change the configuration for Spin application features such as [application variables](https://spinframework.dev/v3/variables),
[key value storage](https://spinframework.dev/v3/kv-store-api-guide), [SQL storage](https://spinframework.dev/v3/sqlite-api-guide)
and [Serverless AI](https://spinframework.dev/v3/serverless-ai-api-guide) at the time you run the application,
...
Why do I need to specify \`--runtime-config-file\` rather than Spin detecting it automatically?

Spin uses safe, local defaults for saving your data. The runtime config file can override those. It should be up to you to opt into that override.

Let’s look at each configuration category in-depth below.

## Application Variables Runtime Configuration

When an application needs the value of an [application variable](https://spinframework.dev/v3/variables), it obtains it from a provider. By default, the only provider Spin considers is the [environment variables provider](https://spinframework.dev/v3/dynamic-configuration#environment-variable-provider). That is, application variables are derived from the environment variables of the Spin process.

You can also tell Spin to use the [Vault provider](https://spinframework.dev/v3/dynamic-configuration#vault-application-variable-provider) and/or the [Azure Key Vault provider](https://spinframework.dev/v3/dynamic-configuration#azure-key-vault-application-variable-provider), by setting these up in the runtime config file.
...
[Using Variables from Applications](https://spinframework.dev/v3/variables#using-variables-from-applications).
...
the [runtime configuration](https://spinframework.dev/v3/dynamic-configuration#runtime-configuration) file:
...
You must individually grant each component access to the stores that it needs to use. To do this, use the `component.key_value_stores` entry in the component manifest within `spin.toml`. See [Spin Key Value Store](https://spinframework.dev/v3/kv-store-api-guide) for more details.
...
By default, components will not have access to any of these databases (even the default one). You must grant each component access to the databases that it needs to use. To do this, use the `component.sqlite_databases` entry in the component manifest within `spin.toml`. See [SQLite Database](https://spinframework.dev/v3/sqlite-api-guide) for more details.
...
By default, components will not have access to the LLM models unless granted explicit access through the `component.ai_models` entry in the component manifest within `spin.toml`. See [Serverless AI](https://spinframework.dev/v3/serverless-ai-api-guide) for more details.
