...
- [Adding Variables to Your Applications](https://spinframework.dev/v3/variables#adding-variables-to-your-applications)
- [Using Variables From Applications](https://spinframework.dev/v3/variables#using-variables-from-applications)
- [Providing Variable Values](https://spinframework.dev/v3/variables#providing-variable-values)
  - [Providing Variable Values from a Secrets Store](https://spinframework.dev/v3/variables#providing-variable-values-from-a-secrets-store)
  - [Providing Variable Values from the Environment](https://spinframework.dev/v3/variables#providing-variable-values-from-the-environment)
  - [Providing Variable Values on the Command Line](https://spinframework.dev/v3/variables#providing-variable-values-on-the-command-line)
- [Troubleshooting](https://spinframework.dev/v3/variables#troubleshooting)
...
These variables are defined in a Spin application manifest (in the `[variables]` section), and their values can be set or overridden at runtime by an [application variables provider](https://spinframework.dev/v3/dynamic-configuration#application-variables-runtime-configuration), or the `--variable` flag to `spin up`. When running Spin locally, the variables provider can be [Hashicorp Vault](https://spinframework.dev/v3/dynamic-configuration#vault-application-variable-provider) for secrets, [Azure Key Vault](https://azure.microsoft.com/en-us/products/key-vault), or host environment variables. [See below](https://spinframework.dev/v3/variables#setting-variable-values) for more information.

## Adding Variables to Your Applications

Variables are added to an application under the top-level `[variables]` section of an application manifest (`spin.toml`). Each entry must either have a default value or be marked as `required = true`. “Required” entries must be [provided](https://spinframework.dev/v3/dynamic-configuration#application-variables-runtime-configuration) with a value.
...
When a component variable references an application variable, its value will dynamically update as the application variable changes. For example, if the `api_token` variable is provided using the [Spin Vault provider](https://spinframework.dev/v3/dynamic-configuration#vault-application-variable-provider), it can be updated by changing the value in HashiCorp Vault. The next time the component gets the value of `token`, the latest value of `api_token` will be returned by the provider. See the [next section](https://spinframework.dev/v3/variables#using-variables-from-applications) to learn how to use Spin’s configuration SDKs to get configuration variables within applications.
...
The `variables` module has a function called `get`(https://spinframework.github.io/spin-python-sdk/v3/variables.html#spin\_sdk.variables.get).
...
You can provide variable values from a secrets store. The Spin CLI supports [Hashicorp Vault](https://spinframework.dev/v3/dynamic-configuration#vault-application-variable-provider) and [Azure Key Vault](https://azure.microsoft.com/en-us/products/key-vault). If you want to do this, you must set up the store in the runtime configuration file, and reference that file using `--runtime-config-file` on the command line.

For information about configuring application variables providers, refer to the [runtime configuration documentation](https://spinframework.dev/v3/dynamic-configuration#application-variables-runtime-configuration).
...
See [Dynamic Application Configuration](https://spinframework.dev/v3/dynamic-configuration#application-variables-runtime-configuration) for information on setting variable values via environment variables, or configuring secure variable providers.
...
To fix this, edit the `spin.toml` and add to the `[component.<component-id>.variables]` table a line such as `<your-variable> = "".` See [above](https://spinframework.dev/v3/variables#adding-variables-to-your-applications) for more information.