...
- [Using HTTP From Applications](https://spinframework.dev/v3/http-outbound#using-http-from-applications)
- [Restrictions](https://spinframework.dev/v3/http-outbound#restrictions)
- [Granting HTTP Permissions to Components](https://spinframework.dev/v3/http-outbound#granting-http-permissions-to-components)
  - [Configuration-Based Permissions](https://spinframework.dev/v3/http-outbound#configuration-based-permissions)
- [Making HTTP Requests Within an Application](https://spinframework.dev/v3/http-outbound#making-http-requests-within-an-application)
  - [Local Service Chaining](https://spinframework.dev/v3/http-outbound#local-service-chaining)
  - [Intra-Application HTTP Requests by Route](https://spinframework.dev/v3/http-outbound#intra-application-http-requests-by-route)
...
- Any type for which you have implemented the [spin\_sdk::http::conversions::TryFromIncomingResponse](https://docs.rs/spin-sdk/5.2.0/spin_sdk/http/conversions/trait.TryFromIncomingResponse.html) trait
...
You can use [application variables](https://spinframework.dev/v3/variables#adding-variables-to-your-applications) in the `allowed_outbound_hosts` field. However, this feature is not yet available on Fermyon Cloud.
...
> Service chaining is the only way to call [private endpoints](https://spinframework.dev/v3/http-trigger#private-endpoints).
...
However, the wildcard implies that the component requires _all other_ components to be local to it. You will therefore not be able to use [selective deployment](https://spinframework.dev/v3/running-apps#splitting-an-application-across-environments) for an application that uses wildcard service chaining.
