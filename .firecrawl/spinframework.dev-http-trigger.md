...
- [Specifying an HTTP Trigger](https://spinframework.dev/v3/http-trigger#specifying-an-http-trigger)
- [HTTP Trigger Routes](https://spinframework.dev/v3/http-trigger#http-trigger-routes)
  - [Resolving Overlapping Routes](https://spinframework.dev/v3/http-trigger#resolving-overlapping-routes)
  - [Private Endpoints](https://spinframework.dev/v3/http-trigger#private-endpoints)
  - [Reserved Routes](https://spinframework.dev/v3/http-trigger#reserved-routes)
- [Authoring HTTP Components](https://spinframework.dev/v3/http-trigger#authoring-http-components)
  - [The Request Handler](https://spinframework.dev/v3/http-trigger#the-request-handler)
  - [Getting Request and Response Information](https://spinframework.dev/v3/http-trigger#getting-request-and-response-information)
  - [Additional Request Information](https://spinframework.dev/v3/http-trigger#additional-request-information)
  - [Inside HTTP Components](https://spinframework.dev/v3/http-trigger#inside-http-components)
- [Static Responses with the HTTP Trigger](https://spinframework.dev/v3/http-trigger#static-responses-with-the-http-trigger)
- [HTTP With Wagi (WebAssembly Gateway Interface)](https://spinframework.dev/v3/http-trigger#http-with-wagi-webassembly-gateway-interface)
  - [Wagi Component Requirements](https://spinframework.dev/v3/http-trigger#wagi-component-requirements)
  - [Request Handling in Wagi](https://spinframework.dev/v3/http-trigger#request-handling-in-wagi)
  - [Wagi HTTP Environment Variables](https://spinframework.dev/v3/http-trigger#wagi-http-environment-variables)
- [Exposing HTTP Triggers Using HTTPS](https://spinframework.dev/v3/http-trigger#exposing-http-triggers-using-https)
  - [Trigger Options](https://spinframework.dev/v3/http-trigger#trigger-options)
  - [Environment Variables](https://spinframework.dev/v3/http-trigger#environment-variables)
...
based on the [application manifest](https://spinframework.dev/v3/writing-apps), it routes them to a
...
Such a trigger says that HTTP requests matching the specified _route_ should be handled by the specified _component_. The `component` field works the same way across all triggers - see [Triggers](https://spinframework.dev/v3/triggers) for the details.
...
To access a private endpoint, use [local service chaining](https://spinframework.dev/v3/http-outbound#local-service-chaining) (where the request is passed in memory without ever leaving the Spin host process). Such calls still require the internal endpoint to be included in `allowed_outbound_hosts`.
...
else — defining the application, running it, or [distributing](https://spinframework.dev/v3/distributing-apps)
...
Wagi supports non-default entry points, and allows you to pass an arguments string that a program can receive as if it had been passed on the command line. If you need these you can specify them in the `executor` table. For details, see the [Manifest Reference](https://spinframework.dev/v3/manifest-reference#the-componenttrigger-table-for-http-applications).
