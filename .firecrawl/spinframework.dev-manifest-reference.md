...
- [Manifest Format](https://spinframework.dev/v3/manifest-reference#manifest-format)
- [Using Variables in the Manifest](https://spinframework.dev/v3/manifest-reference#using-variables-in-the-manifest)
- [Manifest Fields](https://spinframework.dev/v3/manifest-reference#manifest-fields)
- [The `application` Table](https://spinframework.dev/v3/manifest-reference#the-application-table)
- [The `application.trigger` Table](https://spinframework.dev/v3/manifest-reference#the-applicationtrigger-table)
  - [The `application.trigger.redis` Table](https://spinframework.dev/v3/manifest-reference#the-applicationtriggerredis-table)
- [The `variables` Table](https://spinframework.dev/v3/manifest-reference#the-variables-table)
- [The `trigger` Table](https://spinframework.dev/v3/manifest-reference#the-trigger-table)
  - [Common Fields for All `trigger.(type)` Tables](https://spinframework.dev/v3/manifest-reference#common-fields-for-all-triggertype-tables)
  - [Additional Fields for `trigger.http` Tables](https://spinframework.dev/v3/manifest-reference#additional-fields-for-triggerhttp-tables)
  - [Additional Fields for `trigger.redis` Tables](https://spinframework.dev/v3/manifest-reference#additional-fields-for-triggerredis-tables)
- [The `component` Table](https://spinframework.dev/v3/manifest-reference#the-component-table)
- [The `component.(id).build` Table](https://spinframework.dev/v3/manifest-reference#the-componentidbuild-table)
- [Next Steps](https://spinframework.dev/v3/manifest-reference#next-steps)

This page describes the contents of the Spin manifest file, typically called `spin.toml`.

> There are two versions of the manifest format. The manifest format described here (version 2) is recommended if you’re using Spin 2.0 and above. The [previous format (version 1)](https://spinframework.dev/v3/manifest-reference-v1) is supported on Spin 2.x for backward compatibility, and is the only format supported by Spin 1.x.
...
The following fields allow you to use [expressions](https://spinframework.dev/v3/variables#adding-variables-to-your-applications) in their values:
...
| `application` | Required | Table | Information about the application and application-global options. See [The `application` Table](https://spinframework.dev/v3/manifest-reference#the-application-table) below. | `[application]`<br>`name = "greetings"`<br>`version = "1.0.0"` |
| `variables` | Optional | Table | Application configuration variables which the user can set when they run the application. See [The `variables` Table](https://spinframework.dev/v3/manifest-reference#the-variables-table) below. | `[variables]`<br>`message = { default = "hello" }` |
| `trigger` | Required | Table | Associates triggers and conditions to the components that handle them - for example, mapping a HTTP route to a handling component. See [The `trigger` Table](https://spinframework.dev/v3/manifest-reference#the-trigger-table) below. | `[[trigger.http]]`<br>`component = "greeter"`<br>`route = "/greet"` |
| `component` | Required | Table | The WebAssembly components that make up the application, together with whatever configuration and resources they depend on, such as asset files or storage access. See [The `component` Table](https://spinframework.dev/v3/manifest-reference#the-component-table) below. | `[component.greeter]`<br>`source = "greeting_manager.wasm"`<br>`files = ["confetti.jpg"]` |
...
| `trigger` | Optional | Table | Application-global trigger settings. See [The `application.trigger` Table](https://spinframework.dev/v3/manifest-reference#the-applicationtrigger-table) below. | `[application.trigger.redis]`<br>`address = "redis.example.com"` |
...
|  |  | String | This may be an exact route (`/example`), which matches only the given path, or may include wildcards (`/example/:id` or `/example/...`). If two routes overlap, requests are directed to the matching route with the longest prefix. See [the HTTP trigger documentation](https://spinframework.dev/v3/http-trigger) for details and examples. | `"/api/cart/..."` |
|  |  | Table | If the component is a private endpoint used for [local service chaining](https://spinframework.dev/v3/http-outbound#local-service-chaining) then use the table value shown here. | `{ private = true }` |
| `executor` | Optional | Table | How Spin should invoke the component. If present, this is a table. The `type` key is required and may have the values `"spin"` or `"wagi"`. If omitted. the default is `{ type = "spin"}`. See [the HTTP trigger documentation](https://spinframework.dev/v3/http-trigger) for details. | `{ type = "wagi" }` |
...
|  |  |  | If `type = "wagi"`, Spin calls the component’s “main” (`_start`) function using [a CGI-like interface](https://github.com/deislabs/wagi). Components built using languages or toolchains that do not support Wasm interfaces will need to be called in this way. In this case, the following additional keys may be set:<br>\\* `argv` (optional): The string representation of the `argv` list that should be passed into the handler. `${SCRIPT_NAME}` will be replaced with the script name, and `${ARGS}` will be replaced with the query parameters of the request, formatted as arguments. The default is to follow the CGI specification, and pass `${SCRIPT_NAME} ${ARGS}`<br>\\* `entrypoint` (optional): The name of the function to call as the entry point to this handler. By default, it is `_start` (which in most languages translates to `main` in the source code).<br>See [the HTTP trigger documentation](https://spinframework.dev/v3/http-trigger) for details. | `{ type = "wagi" }` |
| `static_response` | Optional | Table | The response Spin should give to requests on this route. A route may not specify both `static_response` and `component`. The response may contain the following fields, all optional:<br>\\* `status_code`: The HTTP status code. Default is 200.<br>\\* `headers`: Table of header names and values. Default is no headers.<br>\\* `body`: The response body. Default is no response body. | { status\_code = 404, body = “not found” } |
...
| `files` | Optional | Array of strings and/or tables | The [files to be made available to the Wasm module at runtime](https://spinframework.dev/v3/writing-apps#including-files-with-components). This is an array, and each element of the array is either: | `[ "images/*.jpg", { source = "assets/images", destination = "/pictures" } ]` |
...
| `allowed_http_hosts` | Optional | Array of strings | The host names or addresses to which the Wasm component is allowed to send HTTP requests. This is retained to simplify transition from the [Version 1 manifest](https://spinframework.dev/v3/manifest-reference-v1); new applications should use `allow_outbound_hosts` instead. | `["example.com", "localhost:8081"]` |
...
| `build` | Optional | Table | The command that `spin build` uses to build this component. See [The `component.(id).build` Table](https://spinframework.dev/v3/manifest-reference#the-componentidbuild-table) below. | `[component.cart.build]`<br>`command = "npm run build"` |
| `variables` | Optional | Table | Application configuration values to be made available to this component. The table keys are user-defined; the values must be strings, and may use template notation as described under [Application Variables](https://spinframework.dev/v3/variables#adding-variables-to-your-applications). | `[component.cart.variables]`<br>`api_base_url = "https://{{ api_host }}/v1"` |
...
- Learn about [writing Spin applications and their manifests](https://spinframework.dev/v3/writing-apps)
- Learn about [dynamic and runtime configuration](https://spinframework.dev/v3/dynamic-configuration)
- See more information about the [HTTP trigger](https://spinframework.dev/v3/http-trigger)
- See more information about the [Redis trigger](https://spinframework.dev/v3/redis-trigger)