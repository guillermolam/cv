...
export default async function initWhoamiMotion(): Promise<void> {
  ...
  const { gsap, ScrollTrigger, SplitText, Flip, Observer } =
  ...
  if (!prefersReduced) {
    ...
    badgeGalleries.forEach((gallery) => {
      const animateActiveBadge = () => {
        ...
        const badgeImage = slide.querySelector<HTMLElement>(
          '.whoami-badge-image',
        );
        ...
        gsap.killTweensOf(
          [orbit, badgeArt, badgeImage, copy, kicker, title, detail].filter(
            Boolean,
          ),
        );
        gsap
          .timeline({ defaults: { ease: 'ds.cinematicReveal' } })
          .fromTo(
            orbit,
            { opacity: 0, rotateX: 72, scale: 0.72 },
            { opacity: 1, rotateX: 62, scale: 1, duration: 0.42 },
            0,
          )
          .fromTo(
            badgeArt,
            { opacity: 0, y: 16, rotateY: -18, rotateX: 12, scale: 0.82 },
            {
              opacity: 1,
              y: 0,
              rotateY: 0,
    ...
    const keyBtns = document.querySelectorAll<HTMLElement>(
      '[data-whoami-keys] [data-whoami-key]',
    );
    if (keyBtns.length) {
      keyBtns.forEach((btn) => {
        btn.addEventListener('click', () => {
          const state = Flip.getState(
            '[data-whoami-keys] [data-whoami-key].active',
          );
