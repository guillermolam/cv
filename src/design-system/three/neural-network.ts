...
export function createNeuralNetwork(
  ...
  const onPointerMove = (e: PointerEvent) => {
    ...
    const hit = (raycaster.intersectObjects(allMeshes(), false)[0]?.object ??
    ...
    edgeGroup.children.forEach((obj, i) => {
      ...
      if (mat.opacity > 0.01) {
        mat.opacity = Math.max(
          0.05,
          mat.opacity * 0.97 +
            0.03 * (0.12 + 0.1 * Math.sin(elapsed * 1.4 + i * 0.65)),
        );
      }
    });
  ...
  const hideAll = () => {
    axonGroup.children.forEach((o) =>
      gsap.to((o as THREE.Mesh).material, { opacity: 0, duration: 0.3 }),
    );
    dendriteGroup.children.forEach((o) =>
    ...
    focusNeuron(id) {
      ...
      edgeGroup.children.forEach((obj) => {
        if ((obj as THREE.Mesh).userData['neuronId'] === id) {
          gsap.to((obj as THREE.Mesh).material, {
            opacity: 0.3,
            duration: 0.5,
            delay,
          });
