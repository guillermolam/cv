...
declare class CardTree extends BaseChart {
  graph: Graph;
  options: TreeOptions;
  private searchControl;
  private breadcrumbControl;
  /**
   * Create a new CardTree instance.
   *
   ...
   * const tree = new CardTree(document.getElementById('chart')!, {
   *   direction: 'top',
   *   nodeWidth: 120,
   *   nodeHeight: 40,
   * });
  ...
  /**
   * Ensure the watermark is not shown.
   */
...
declare class Graph extends Paper {
   ...
   * lineage back to root, tags matched nodes with `data-cardtree-match`.
...
/**
 * Recursive node structure passed to `CardTree.render()`.
 *
...
export declare interface NodeOptions {
  ...
  /** CSS class name added to every node element. @default 'cardtree-node' */
...
declare class Paper {
  ...
  static drawTemplate(
    template: string,
    { nodeHeight, nodeWidth }?: Partial<NodeOptions>,
  ): ForeignObject;
  static drawText(
    text: string | undefined,
    { dx, dy, x, y }: Partial<TextAttr>,
  ): Text_2;
   ...
   */
  setTreeA11yAttributes(
    label: string,
    selectionMode?: 'multi' | 'single' | false,
  ): void;
}
...
export declare interface TooltipOptions {
  ...
  /** HTML `id` for the tooltip container element. @default 'cardtree-tooltip-container' */
...
/**
 * Full configuration type for `CardTree`. An intersection of all sub-option
 ...
 */
export declare type TreeOptions = CommonOptions &
  EdgeOptions &
  FontOptions &
  NodeOptions &
  TooltipOptions;
