...
export const getEntryLang = (entry: { data?: { lang?: string } }) =>
  entry.data?.lang;

export const getEntryStableIdForCollection = (
  collection: SupportedCollectionName,
  entry: { id: string; data: any },
) => {
  const data = entry.data ?? {};

  if (collection === 'profile') return entry.id.split('/').at(-1) ?? entry.id;
  if (collection === 'contactChannels')
...
export const getEntrySlugForCollection = (
  collection: SupportedCollectionName,
  entry: { id: string; data: any },
) => {
  const data = entry.data ?? {};

  if (collection === 'caseStudies' && typeof data.slug === 'string')
    return data.slug;
  if (collection === 'blog' && typeof data.blogSlug === 'string')
    return data.blogSlug;
  if (
...
export const getEntryTitleForCollection = (
  collection: SupportedCollectionName,
  entry: { data: any },
) => {
  const data = entry.data ?? {};
  ...
  if (collection === 'experience')
    return `${data.companyName ?? 'Experience'} — ${data.roleTitle ?? ''}`.trim();
...
export const getAllContentNodes = async () => {
  const entriesByCollection = await Promise.all(
    supportedCollections.map(
      async (collection) =>
        [collection, await getCollectionEntries(collection)] as const,
    ),
  );

  return Object.fromEntries(entriesByCollection) as Record<
