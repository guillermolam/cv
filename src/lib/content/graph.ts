...
export const getNodeKey = (collection: string, id: string) =>
  `${collection}:${id}`;
...
export const buildNodesFromEntries = (
  entriesByCollection: Record<string, Array<{ id: string; data: any }>>,
) => {
    ...
    for (const entry of entries) {
      ...
      const title =
        ...
        (collection === 'experience' &&
          `${data.companyName ?? 'Experience'} — ${data.roleTitle ?? ''}`.trim()) ||
...
export const buildEdgesFromEntries = (
  entriesByCollection: Record<string, Array<{ id: string; data: any }>>,
) => {
  ...
  for (const [collection, entries] of Object.entries(entriesByCollection)) {
    for (const entry of entries) {
      ...
      if (Array.isArray(data.links)) {
        for (const link of data.links) {
          ...
          edges.push({
            ...
            context:
              typeof link.context === 'string' ? link.context : undefined,
            source: 'explicit',
          });
        }
      }
      ...
      if (
        ...
        addReferenceEdges(
          fromKey,
          data.featured.projectIds,
          'projects',
          'references',
        );
        addReferenceEdges(
          fromKey,
          data.featured.caseStudyIds,
          'caseStudies',
          'references',
        );
        addReferenceEdges(
          fromKey,
          data.featured.blogSlugs,
          'blog',
          'references',
        );
      }
