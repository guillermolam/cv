...
export const validateNoDuplicateNodeKeys = (
  graph: ContentGraph,
): ContentGraphIssue[] => {
  ...
  for (const node of graph.nodes) {
    ...
    if (
      ...
      issues.push({
        ...
        recommendedFix:
          'Rename the entry ID to kebab-case and update all references.',
      });
    }
  }
...
export const validateNoMissingTargets = (
  graph: ContentGraph,
): ContentGraphIssue[] => {
  const issues: ContentGraphIssue[] = [];
  ...
  for (const edge of graph.edges) {
    if (!keys.has(edge.to)) {
      issues.push({
        ...
        recommendedFix:
          'Create the missing entry or correct the targetId/targetCollection.',
      });
    }
  }
  ...
  for (const node of graph.nodes) {
    ...
    if (
      ...
      issues.push({
        ...
        recommendedFix:
          'Set availability=coming-soon or add downloadPath pointing to an existing file under public/cv/.',
      });
    }
  }
    ...
    if (data.status === 'active') {
      ...
      if (!hasEvidence) {
        issues.push({
          ...
          nodeKey: node.key,
          message:
            'certifications status=active but no evidence fields are present (credentialUrl/credentialId/issuedDate).',
          recommendedFix:
            'Add credentialUrl/credentialId/issuedDate, or set status=in-progress/unknown.',
        });
      }
    }
  }
...
export const validateSecurityFacetTargets = (
  ...
  const categoryIds = new Set(
    graph.nodes.filter((n) => n.collection === 'categories').map((n) => n.id),
  );
  const nodeDataByKey = context?.nodeDataByKey ?? {};
    ...
    for (const id of ids) {
      if (!categoryIds.has(id)) {
        issues.push({
          ...
          recommendedFix:
            'Create the missing category entry or remove the invalid security facet ID.',
        });
      }
    }
  };
  ...
  for (const node of urlNodes) {
    if (node.url && isUnsafeUrl(node.url)) {
      issues.push({
        ...
        recommendedFix:
          'Replace the URL with a safe https:// or mailto: link, or remove it.',
      });
    }
  }
  ...
  for (const u of context?.externalUrls ?? []) {
    if (isUnsafeUrl(u.url)) {
      issues.push({
        ...
        recommendedFix:
          'Replace the URL with a safe https:// link, or remove it.',
      });
    }
  }
