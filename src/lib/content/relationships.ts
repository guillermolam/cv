...
const getNode = (graph: ContentGraph, nodeKey: string) =>
  graph.nodes.find((n) => n.key === nodeKey);
...
export const getRelatedEntries = (
  ...
  for (const other of graph.nodes) {
    ...
    if (sharedCount(node.skillIds, other.skillIds) > 0)
      relatedKeys.add(other.key);
    if (sharedCount(node.toolIds, other.toolIds) > 0)
      relatedKeys.add(other.key);
    if (sharedCount(node.categoryIds, other.categoryIds) > 0)
      relatedKeys.add(other.key);
...
export const getRelatedBySharedTags = (
  nodeKey: string,
  graph: ContentGraph,
) => {
  const node = getNode(graph, nodeKey);
  ...
  return graph.nodes.filter(
    (n) => n.key !== nodeKey && sharedCount(node.tagIds, n.tagIds) > 0,
  );
};

export const getRelatedBySharedCategories = (
  nodeKey: string,
  graph: ContentGraph,
) => {
  const node = getNode(graph, nodeKey);
  ...
  return graph.nodes.filter(
    (n) =>
      n.key !== nodeKey && sharedCount(node.categoryIds, n.categoryIds) > 0,
  );
};

export const getRelatedBySharedTools = (
  nodeKey: string,
  graph: ContentGraph,
) => {
  const node = getNode(graph, nodeKey);
  ...
  return graph.nodes.filter(
    (n) => n.key !== nodeKey && sharedCount(node.toolIds, n.toolIds) > 0,
  );
};

export const getRelatedBySharedSkills = (
  nodeKey: string,
  graph: ContentGraph,
) => {
  const node = getNode(graph, nodeKey);
  ...
  return graph.nodes.filter(
    (n) => n.key !== nodeKey && sharedCount(node.skillIds, n.skillIds) > 0,
  );
};

export const scoreRelatedEntries = (
  nodeKey: string,
  graph: ContentGraph,
): RelatedEntryScore[] => {
  const node = getNode(graph, nodeKey);
  ...
  const explicitTargets = new Set(
    outgoing.filter((e) => e.source === 'explicit').map((e) => e.to),
  );
  const explicitSources = new Set(
    incoming.filter((e) => e.source === 'explicit').map((e) => e.from),
  );

  const candidates = uniq([
    ...explicitTargets,
    ...explicitSources,
    ...getRelatedEntries(nodeKey, graph).map((n) => n.key),
  ])
    .filter((k) => k !== nodeKey)
  ...
  for (const other of candidates) {
    ...
    const explicitEdgeCount = graph.edges.filter(
      (e) =>
        (e.from === nodeKey && e.to === other.key) ||
        (e.to === nodeKey && e.from === other.key),
    ).length;
    ...
    if (score > 0)
      scores.push({ nodeKey: other.key, score, reasons: uniq(reasons) });
  }
