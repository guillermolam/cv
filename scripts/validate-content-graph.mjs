...
const buildGraph = async () => {
  ...
  const addExternalUrl = (nodeKey, field, url) => {
    if (typeof url === 'string' && url.length > 0)
      externalUrls.push({ nodeKey, field, url });
  };
  ...
  for (const filePath of markdownFiles) {
    ...
    if (typeof stableId !== 'string' || stableId.length === 0)
      throw new Error(`Missing stable ID: ${relPath}`);

    const slug =
      (collection === 'caseStudies' && typeof data.slug === 'string'
        ? data.slug
        : undefined) ??
      (collection === 'blog' && typeof data.blogSlug === 'string'
        ? data.blogSlug
        : undefined) ??
      stableId;
    ...
    const title =
      ...
      (collection === 'experience' &&
        (data.roleTitle
          ? `${data.companyName ?? 'Experience'} — ${data.roleTitle}`
          : undefined)) ||
    ...
    if (Array.isArray(data.proofLinks))
      for (const p of data.proofLinks ?? [])
        addExternalUrl(key, 'proofLinks.url', p?.url);
    if (Array.isArray(data.channels))
      for (const c of data.channels ?? [])
        addExternalUrl(key, 'channels.url', c?.url);
    if (Array.isArray(data.links))
      for (const l of data.links ?? [])
        addExternalUrl(key, 'links.context', l?.context);
    ...
    if (
      ...
      if (Array.isArray(f.projectIds))
        for (const id of f.projectIds)
          edges.push({
            from: key,
            to: getNodeKey('projects', id),
            type: 'featuredProject',
            source: 'reference',
          });
      if (Array.isArray(f.caseStudyIds))
        for (const id of f.caseStudyIds)
          edges.push({
            from: key,
            to: getNodeKey('caseStudies', id),
            type: 'featuredCaseStudy',
            source: 'reference',
          });
      if (Array.isArray(f.blogSlugs))
        for (const id of f.blogSlugs)
          edges.push({
            from: key,
            to: getNodeKey('blog', id),
            type: 'featuredBlog',
            source: 'reference',
          });
    }
...
const validateGraph = async () => {
  const { nodes, edges, nodeDataByKey, fileByNodeKey, externalUrls } =
    await buildGraph();
  ...
  const addIssue = (severity, nodeKey, message, recommendedFix) => {
    issues.push({
      severity,
      nodeKey,
      file: nodeKey ? fileByNodeKey[nodeKey] : undefined,
      message,
      recommendedFix,
    });
  };

  const addWarning = (nodeKey, message) =>
    warnings.push({
      nodeKey,
      file: nodeKey ? fileByNodeKey[nodeKey] : undefined,
      message,
  ...
  for (const node of nodes) {
    if (seen.has(node.key))
      addIssue(
        ...
        'Ensure IDs are unique per collection.',
      );
    seen.add(node.key);
    if (!kebab.test(node.id))
      addIssue(
        'blocker',
        node.key,
        `Non-kebab stable ID "${node.id}".`,
        'Rename to kebab-case and update references.',
      );
  }
  ...
  for (const edge of edges) {
    if (!nodeKeys.has(edge.to))
      addIssue(
        'blocker',
        edge.from,
        `Missing target "${edge.to}" referenced by "${edge.type}".`,
        'Create the target entry or fix the reference.',
      );
  }
  ...
  for (const { nodeKey, field, url } of externalUrls) {
    if (isUnsafeUrl(url))
      addIssue(
        'blocker',
        nodeKey,
        `Unsafe URL scheme detected in ${field}: "${url}".`,
        'Replace with a safe https:// URL or remove it.',
      );
  }
  ...
  for (const node of nodes) {
    ...
    if (node.collection === 'cvFormats' && data.availability === 'available') {
      if (
        ...
        data.downloadPath.length === 0
      ) {
        addIssue(
          'blocker',
          node.key,
          'cvFormats availability=available but downloadPath is missing.',
          'Add downloadPath or set availability=coming-soon.',
        );
      } else {
        const resolved = path.resolve(
          publicRoot,
          data.downloadPath.replace(/^\//, ''),
        );
        try {
          ...
          addIssue(
            'blocker',
            node.key,
            `cvFormats downloadPath does not exist: ${data.downloadPath}`,
            'Add the file under public/cv/ or set availability=coming-soon.',
          );
        }
      }
    }
    ...
    if (security && typeof security === 'object') {
      const categoryIds = new Set(
        nodes.filter((n) => n.collection === 'categories').map((n) => n.id),
      );
      const facetFields = [
        'domainIds',
        'subdomainIds',
        ...
        for (const id of security[f]) {
          ...
          if (!categoryIds.has(id)) {
            addIssue(
              'blocker',
              node.key,
              `Security facet "${f}" references missing categoryId "${id}".`,
              'Create the category entry or remove the invalid facet ID.',
            );
          }
        }
      }
  ...
  for (const node of nodes) {
    if (!(outCount.get(node.key) ?? 0) && !(inCount.get(node.key) ?? 0))
      addWarning(node.key, 'Orphan node (no incoming/outgoing edges).');
  }

  const docsWarnings = [];
  try {
    const contentModel = await fs.readFile(
      path.resolve('docs/architecture/content-model.md'),
      'utf8',
    );
    if (
      contentModel.includes('pubDate') ||
      contentModel.includes('src/content/config.ts') ||
      contentModel.includes('tags: string[]')
    ) {
      docsWarnings.push(
        'docs/architecture/content-model.md appears out of date vs current schemas (pubDate/src/content/config.ts/freeform tags).',
      );
    }
  } catch {}
