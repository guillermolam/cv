...
const ME_SUBFOLDERS = new Set(['elearning', 'projects', 'cv | resume']);
...
const inferType = (folderPath) => {
  ...
  if (
    lp.includes('course') ||
    lp.includes('elearning') ||
    lp.includes('training')
  )
    return 'course';
  ...
  if (lp.includes('doc') || lp.includes('documentation'))
...
const inferCategories = (folderPath) => {
  const lp = folderPath.toLowerCase();
  const cats = [];
  if (lp.includes('security') || lp.includes('appsec') || lp.includes('siem'))
    cats.push('cloud-security');
  if (lp.includes('kubernetes') || lp.includes('k8s'))
    cats.push('kubernetes-platform');
  if (lp.includes('gitops') || lp.includes('argocd') || lp.includes('ci/cd'))
    cats.push('gitops-delivery');
  if (lp.includes('devsecops')) cats.push('devsecops');
  if (lp.includes('terraform') || lp.includes('iac') || lp.includes('infra'))
    cats.push('platform-engineering');
  if (lp.includes('supply') || lp.includes('sbom'))
    cats.push('supply-chain-security');
  if (lp.includes('frontend') || lp.includes('ux'))
    cats.push('creative-frontend');
  if (lp.includes('cloud') || lp.includes('aws') || lp.includes('azure'))
    cats.push('cloud-security');
  if (lp.includes('ai') || lp.includes('ml') || lp.includes('genai'))
    cats.push('ai-security');
  return [...new Set(cats)];
};
...
async function writeMocFile(folderName, children, outputDir) {
  ...
  if (subfolders.length > 0) {
    ...
    for (const sf of subfolders) {
      lines.push(
        `- 📁 [[_MOC ${sanitizeFilename(sf.name)}|${sf.name}]] (${(sf.children ?? []).length})`,
      );
    }
    lines.push('');
  }
...
async function main() {
  ...
  if (!DRY_RUN) {
    await fs.writeFile(
      path.join(VAULT_ROOT, '_MOC Knowledge Base.md'),
      rootMocContent,
      'utf8',
    );
  }
