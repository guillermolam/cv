...
const inferInternalUrl = (node) => {
  ...
  if (
    node.collection === 'contactChannels' ||
    node.collection === 'socialLinks'
  )
    return `${langPrefix}/contact`;
  if (node.collection === 'blog') return `${langPrefix}/blog/${node.slug}`;
  if (node.collection === 'caseStudies')
    return `${langPrefix}/case-studies/${node.slug}`;
  if (node.collection === 'knowledgeResources')
    return `${langPrefix}/knowledge/${node.slug}`;
  return undefined;
};
...
const buildGraph = async () => {
  ...
  for (const filePath of markdownFiles) {
    ...
    if (!kebab.test(stableId))
      warnings.push({
        message: `Non-kebab id detected (will fail validation): ${collection}:${stableId}`,
        file: relPath,
      });
    ...
    if (
      ...
      warnings.push({
        nodeKey: node.key,
        file: relPath,
        message: 'Entry contains needsConfirmation.',
      });
    }
  ...
  for (const n of nodes)
    collectionCounts[n.collection] = (collectionCounts[n.collection] ?? 0) + 1;
  ...
  for (const e of edges) {
    if (!nodeKeys.has(e.to))
      warnings.push({
        message: `Edge target missing (will fail validation): ${e.from} -> ${e.to}`,
        type: e.type,
      });
  ...
  for (const n of nodes) {
    if (!(outCount.get(n.key) ?? 0) && !(inCount.get(n.key) ?? 0))
      warnings.push({
        nodeKey: n.key,
        message: 'Orphan node (no incoming/outgoing edges).',
      });
  }
