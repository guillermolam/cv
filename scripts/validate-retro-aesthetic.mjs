...
const main = async () => {
  ...
  const hasKeyboardCss = Object.prototype.hasOwnProperty.call(
    deps,
    'keyboard-css',
  );
  ...
  const missingDepthTokens = requiredDepthTokens.filter(
    (t) => !depth.includes(t),
  );
  ...
  const uiButtonHasVariant =
    uiButton.includes("'keycap'") &&
    uiButton.includes("'terminal'") &&
    uiButton.includes("'control'");
  const catalogHasExamples =
    catalog.toLowerCase().includes('retro computer aesthetic') &&
    catalog.toLowerCase().includes('keycap buttons');
